...
df = pd.read_csv("/scratch/sciteam/ILL_bbdv/NEON/data/NEON.DP1.10026.001.cfc_elements.expanded.csv")
...
for i in range(len(df)):
    ...
    p = sorted(glob("/scratch/sciteam/ILL_bbdv/NEON/DP3.30006.001/*/{yymm}/{fid}_*.tif".format(yymm=yymm, fid=fid)))
    if len(p) == 0:
...
rows.to_csv(
    "/scratch/sciteam/ILL_bbdv/NEON/data/NEON.DP1.10026.001.cfc_elements.expanded.spectra.csv",
    index=False
)
