...
def calc_resample_coeffs(
        in_wave: np.ndarray, in_fwhm: np.ndarray, out_wave: np.ndarray, out_fwhm: np.ndarray, spacing=1
) -> np.ndarray:
    """Given a set of source and destination wavelengths and FWHMs this
    function calculates the relative contribution or each input wavelength
    to the output wavelength. It assumes that both input and output
    response functions follow a gaussian distribution.
    All inputs should be provide in nanometers.
    Args:
    ...
    Returns:
        numpy.ndarray: Transform coefficients.
    """

    out_matrix = []
    min_spectrum = min(np.min(out_wave), np.min(in_wave)) // 100 * 100 - 100
    max_spectrum = 100 + max(np.max(out_wave), np.max(in_wave)) // 100 * 100
...
def apply_resampler(hy_obj, data):
    ...
    if hy_obj.resampler['type'] == 'gaussian':
        ...
        interp_func = interp1d(
            hy_obj.wavelengths[~hy_obj.bad_bands], data,
